...
import { DiffFileSummary, SafetyWarning } from "./types";
...
export function showSafetyNotifications(warnings: SafetyWarning[]): void {
  const critical = warnings.filter((w) => w.severity === "critical");
  const warningLevel = warnings.filter((w) => w.severity === "warning");

  if (critical.length > 0) {
    vscode.window.showWarningMessage(
      `Worktrace Safety: ${critical.length} critical issue(s) detected. Check session summary for details.`
    );
  } else if (warningLevel.length > 0) {
    vscode.window.showInformationMessage(
      `Worktrace Safety: ${warningLevel.length} warning(s) found. Check session summary for details.`
    );
  }
}
